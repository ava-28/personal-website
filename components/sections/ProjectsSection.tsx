...
function ProjectThumbnail({ project }: { project: Project }) {
  if (!SHOW_GENERATED_ART && project.images && project.images.length > 0) {
    return <img src={project.images[0]} alt={project.title} className="h-full w-full object-cover" />
  }
  if (project.customVisual) {
    return (
      <>
        {project.customVisual === 'statarb' && <StatArbVisual />}
        {project.customVisual === 'attention' && <AttentionVisual />}
        {project.customVisual === 'trading' && <TradingVisual />}
        {project.customVisual === 'montecarlo-gen' && <MonteCarloGenVisual />}
        {project.customVisual === 'gridworld-gen' && <GridworldGenVisual />}
        {project.customVisual === 'hydro-gen' && <HydroGenVisual />}
      </>
    )
  }
  return <ProjectVisual kind={project.visual} />
}

export function ProjectsSection() {
  return (
    <section id="projects" className="section-block mb-16">
      <h2 className="mb-6 text-lg font-bold text-stone-900 dark:text-white">Projects</h2>
      <div className="space-y-6">
        {projects.map((project) => (
          <article
            key={project.title}
            className="entry-row grid grid-cols-[96px_1fr] gap-5 sm:grid-cols-[168px_1fr]"
          >
            <div className="aspect-square w-full overflow-hidden rounded-lg border border-stone-200 dark:border-slate-700">
              <ProjectThumbnail project={project} />
            </div>
            <div>
              {project.subtitle && (
                <p className="mb-1 text-[13px] text-stone-500 dark:text-slate-400">{project.subtitle}</p>
              )}
              <h3 className="mb-2 text-base font-bold text-stone-900 dark:text-white">{project.title}</h3>
              {project.repo && (
                <p className="mb-2 text-[13px] text-stone-500 dark:text-slate-400">
                  Code:{' '}
                  <a
                    href={project.repo}
                    target="_blank"
              ...
              <p className="text-[15px] leading-relaxed text-stone-600 dark:text-slate-300">
                {project.description}
              </p>
            </div>
          </article>
